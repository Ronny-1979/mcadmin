...
async function uploadWorld(inp){const f=inp.files[0];if(!f)return;pendingWorldFile=f;inp.value='';toast('Importiere Welt...','info');try{const r=await api('upload_world',{},{world:f});if(r.compatibility_warning){showWorldCompatModal(r.issues,r.experiments);return;}pendingWorldFile=null;toast(r.message||(r.success?'Welt importiert':'Fehler'),r.success?'success':'error');if(r.success)loadWorlds();}catch(err){toast('Upload fehlgeschlagen: '+err.message,'error');}}
function showWorldCompatModal(issues,experiments){var ul=document.getElementById('compat-issues-list');ul.innerHTML='';(issues||[]).forEach(function(i){var li=document.createElement('li');li.textContent=i.type+'-Pack "'+i.pack+'": erfordert '+i.requires+', installiert '+i.installed;ul.appendChild(li);});document.getElementById('compat-issues-section').style.display=(issues&&issues.length)?'':'none';var expUl=document.getElementById('compat-exp-list');expUl.innerHTML='';(experiments||[]).forEach(function(name){var li=document.createElement('li');li.textContent=name;expUl.appendChild(li);});document.getElementById('compat-exp-section').style.display=(experiments&&experiments.length)?'':'none';openModal('modal-world-compat');}
